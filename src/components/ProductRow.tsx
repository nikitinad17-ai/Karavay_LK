import { useStore, useCart } from '../store';
import { esc, fmtMoney, isPromo, promoOldPrice } from '../utils';
...
export default function ProductRow({ p }: { p: Product }) {
  ...
  function step(which: 'lots' | 'pcs', dir: '+' | '-') {
  ...
  function onLotsInput(e: ChangeEvent<HTMLInputElement>) {
    const v = Math.max(0, Math.floor(Number(e.target.value) || 0));
    setCartDetail(p, v, d.pcs || 0);
  }
  function onPcsInput(e: ChangeEvent<HTMLInputElement>) {
    const v = Math.max(0, Math.floor(Number(e.target.value) || 0));
  ...
  return (
    <div className={'product' + (promo ? ' product--promo' : '') + (validationError ? ' product--invalid' : '')}>
      ...
      <div style={{ textAlign: 'right', minWidth: 120 }}>
        ...
        {discount > 0 ? <div className={'price-disc' + (promo ? ' price-disc--promo' : '')}>−{discount.toFixed(1)}%</div> : null}
