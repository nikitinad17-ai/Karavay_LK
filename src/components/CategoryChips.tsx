...
import type { OrderFilter, Product } from '../types';

function matchTypeOnly(_p: Product, f: OrderFilter): boolean {
  if (f === 'in-cart') return false; // считаем ниже отдельно через state.cart
  return true;
}

interface CategoryStat { name: string; order: number; count: number }

export function categories(products: Product[]): CategoryStat[] {
  const cats: Record<string, CategoryStat> = {};
  products.forEach((p) => {
    if (!cats[p.category]) cats[p.category] = { name: p.category, order: p.categoryOrder ?? 999, count: 0 };
    cats[p.category].count++;
  });
  return Object.keys(cats).map((k) => cats[k]).sort((a, c) => a.order - c.order || a.name.localeCompare(c.name, 'ru'));
}

export function matchType(p: Product, f: OrderFilter, cart: Record<string, number>): boolean {
  if (f === 'in-cart') return !!cart[p.id];
  if (f === 'promo') return !!p.isPromo;
  return true;
}
export function matchSearch(p: Product, q: string): boolean {
  if (!q) return true;
  return p.name.toLowerCase().indexOf(q) >= 0 || String(p.code).indexOf(q) >= 0;
}
export function matchCats(p: Product, selectedCats: string[]): boolean {
  return !selectedCats.length || selectedCats.indexOf(p.category) >= 0;
}

export default function CategoryChips({ q }: { q: string }) {
  ...
  function toggleCat(name: string, checked: boolean) {
