...
export function StoreProvider({ children }: StoreProviderProps) {
  ...
  const requestGateRef = useRef<ReturnType<typeof createRequestGate> | null>(null);
  ...
  const patch = useCallback((payload: StatePatch) => {
  ...
  const isLatestRequest = useCallback((key: string, id: number) => requestGateRef.current!.isLatest(key, id), []);
  const value: StoreContextValue = { state, patch, dispatch, getState, beginRequest, isLatestRequest };
...
export function useToast() {
  ...
  return useCallback((msg: string, type: ToastMessage['type'] = '') => {
...
export function currentOutletOf(state: AppState): Outlet | null {
...
export function lastOrderForOutlet(state: AppState, outletId: number): Order | null {
...
export function useLoaders() {
  ...
  const loadMatrix = useCallback(() => {
    ...
    return api<MatrixResponse>(url).then((resp) => {
      ...
      const ids: Record<string, true> = {}; products.forEach((p) => { ids[p.id] = true; });
      const cur = getState();
      ...
      patch({ products: [], matrixLoading: false, matrixError: errorDetail(err) || 'Не удалось получить матрицу продукции' });
    });
  ...
  const loadBuyerData = useCallback(() => {
    ...
    return Promise.all([
      api<OrdersResponse>('/api/v1/payers/' + idPay + '/orders'),
      api<DocumentRecord[]>('/api/buyers/' + bid + '/documents').catch(() => [] as DocumentRecord[]),
      ...
      const patchObj: StatePatch = { ordersAll, documents: docs || [], loading: false };
      ...
      patch({
        loading: false,
        initialLoadError: errorDetail(err) || 'Не удалось загрузить данные кабинета. Проверьте соединение и повторите попытку.',
      });
    });
  }, [patch, getState, loadMatrix, beginRequest, isLatestRequest]);

  const refreshSetupLastOrder = useCallback((ol: Outlet | null) => {
    const state = getState();
    ...
    return api<OrdersResponse>('/api/v1/clients/' + idClt + '/orders?last=1').then((resp) => {
...
export function useAuth() {
  ...
  const applyLoginResponse = useCallback((resp: LoginResponse, loadBusinessData = true) => {
    ...
    if (resp.role === 'buyer') {
      ...
      if (!resp.client) throw new AuthError('INVALID_RESPONSE', 'В ответе для точки отсутствуют её данные.', 502);
...
export function useCart() {
  ...
  const cartDetail = useCallback((pid: number): CartDetail => getState().cartDetails[pid] || { lots: 0, pcs: 0 }, [getState]);

  const setCartDetail = useCallback((p: Product, lots: number, pcs: number) => {
    const state = getState();
    const pid = p.id;
  ...
  const applyOrderToCart = useCallback((order: Order) => {
...
function errorDetail(value: unknown): string {
  return isApiError(value) && typeof value.detail === 'string' ? value.detail : '';
}

function kisError(err: unknown, fallback: string): string {
...
export function useOrderActions() {
  ...
  const loadOrderDetails = useCallback((o: Order, cb?: (order: Order) => void): Promise<Order> | void => {
    if (!o) return;
    ...
    return api<OrderDetailsResponse>('/api/v1/orders/' + o.id).then((resp) => {
      ...
      const patchObj: StatePatch = { detailsCache };
  ...
  const openOrderModal = useCallback((o: Order) => {
    if (!o) return;
  ...
  const applyOrderItemQty = useCallback((order: Order, it: OrderItem, newQty: number) => {
  ...
  const saveOrderEdits = useCallback((o: Order, _snapshot: OrderSnapshot | null) => {
    if (!o) return;
    ...
    return api<OrderMutationResponse>('/api/v1/orders/' + o.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ Product: products }) })
      .then((resp) => {
        const si = stateInfo(resp.State);
        const saved = {
          ...cloneOrder(o), totalUnits: resp.KolSht ?? o.totalUnits, total: resp.SumAll ?? o.total,
  ...
  const cancelOrder = useCallback((o: Order) => {
    if (!o) return;
    return api<OrderMutationResponse>('/api/v1/orders/' + o.id, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } })
      .then((resp) => {
        const si = stateInfo(resp.State);
  ...
  const askCancelOrder = useCallback((o: Order) => {
    patch({
      confirm: {
  ...
  const submitOrder = useCallback(() => {
    ...
    const ol = currentOutletOf(state);
    if (!ol) {
      patch({ reviewOpen: true, reviewError: 'Выберите получателя заказа.' });
      return Promise.resolve(false);
    }
    ...
    return api<KisOrder>('/api/v1/orders/', {
  ...
  const repeatOrderFromHistory = useCallback((order: Order) => {
    const state = getState();
    ...
    if (order.group != null) patchObj.orderGroup = order.group === 0 ? 0 : 1;
  ...
  const repeatLastOrder = useCallback(() => {
    ...
    loadOrderDetails(last, () => {
      ...
      const patchObj: StatePatch = { orderReady: true };
      if (last.group != null) patchObj.orderGroup = (last.group === 0 ? 0 : 1) as OrderGroup;
