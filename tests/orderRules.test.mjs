...
} from '../src/orderRules.js';
import { createRequestGate } from '../src/requestGate.js';

const outlet = { id: 1, minOrderSum: 100, name: 'Точка', code: 'M-1' };
const products = [
  { id: 1, code: 'P-1', name: 'Хлеб', price: 25, piecesPerLot: 10, minOrder: 5, lotOnly: false },
  { id: 2, code: 'P-2', name: 'Лоточный товар', price: 10, piecesPerLot: 6, minOrder: 6, lotOnly: true },
];

function state(cart) {
  return { cart, products, buyer: { minOrderSum: 50 }, orderDate: '2026-09-14', matrixLoading: false, matrixError: '' };
}
...
test('редактирование заказа создаёт новую структуру и не мутирует исходник', () => {
  const original = { id: 7, totalUnits: 2, total: 50, items: [{ id_prd: 1, qty: 2, price: 25, sum: 50 }] };
  const snapshot = cloneOrder(original);
