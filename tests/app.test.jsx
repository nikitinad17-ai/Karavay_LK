...
async function openCatalog(user) {
...
describe('React 3.3', () => {
  test('открывает проверку заказа и возвращается без потери корзины', async () => {
    ...
    const plus = product.querySelector('button[aria-label="+"]');
    await user.click(plus);
    const totalBefore = product.querySelector('.qty__total').textContent;
    await user.click(document.querySelector('#sumSubmit'));
  ...
  test('не закрывает изменённый заказ без подтверждения', async () => {
    ...
    const firstRow = await waitFor(() => {
      const row = document.querySelector('.orders-history__table tbody tr');
    ...
    await user.click(dialog.querySelector('button[aria-label="+"]'));
